    ...
    def _guard_command(
        ...
        *,
        ...
        if not explicitly_allowed:
            ...
            # 上面的 deny_patterns 只挡带 flag 的变体（`rm -rf` / `del /f` /
            # `rmdir /s`）；裸 `rm x`、`del x`、`Remove-Item x`、`unlink x`
            # 依旧会**真删**。这里把所有删除动词统一收口：OS 临时目录例外
            # （走真删），其余一律拦下并指回 `delete_file` 工具——后者把数据
            # 送进系统回收站，且回收站不可用时 fail-closed。
            # 与 deny_patterns 同层：`allow_patterns` 仍是运维显式放行的出口。
            if is_delete_command(cmd) and not delete_command_is_temp_only(cmd):
                return ToolResult.error(
                    "Error: Command blocked by the safe-delete guard"
                    + DELETE_REDIRECT_NOTE
                )
