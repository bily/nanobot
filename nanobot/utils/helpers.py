...
def sync_workspace_templates(workspace: Path, silent: bool = False) -> list[str]:
    ...
    # [LOCAL PATCH] 提示词外置（§13.4）：把**可覆盖**的主系统提示词默认副本也落到
    # prompts/ 下。不做这一步，「改提示词不重编译」就是一句空话——用户根本不知道
    # 该建哪些文件、名字叫什么。_write 只在目标缺失时写入，因此用户改过的副本
    # 不会被后续 init 还原。
    from nanobot.utils.prompt_templates import OVERRIDABLE_TEMPLATES

    for _tpl_name in OVERRIDABLE_TEMPLATES:
        _write(tpl / _tpl_name, workspace / "prompts" / _tpl_name)
