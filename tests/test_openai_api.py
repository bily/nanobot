...
def _make_mock_agent(response_text: str = "mock response") -> MagicMock:
    ...
    # 模型预设/解析器是 LOCAL PATCH 引入的可选配置。MagicMock 会把它们自动
    # 造成真值对象，导致 /v1/models 返回空目录、model 校验走错分支——
    # 这里显式置空，让 mock 等价于「未配置预设」的真实网关。
    agent.model_presets = None
    agent.runtime_resolver = None
    return agent
