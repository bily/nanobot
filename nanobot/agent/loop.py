...
from nanobot.agent.skills import default_user_skills_dir
...
class AgentLoop:
    ...
    def __init__(
        ...
        # [LOCAL PATCH] FR-3.4：用户级插件目录要存到实例上。
        # `_register_default_tools` 是另一个方法，够不到 `__init__` 的局部变量；
        # 而文件工具的读闸口必须拿到与 SkillsLoader 完全相同的值（见
        # ToolContext.user_plugins_dir 的说明）。
        ...
        self.context = ContextBuilder(
            ...
            # FR-3.1 用户级技能目录：默认由 from_config 注入产品路径；
            # 直接 `Agent(...)` 构造（多为单测）时为 None = 不扫描。
            user_skills_dir=user_skills_dir,
            # FR-3.4 用户级插件目录：同上，只在 from_config 注入。
    ...
    def from_config(
        ...
        **extra: Any,
        ...
        """
        # 局部导入是**必需的**，不是风格问题：`nanobot.agent.plugins` 会 import
        # `nanobot.config.loader`，而本模块经 `nanobot.agent.__init__` 被
        # `config.schema` 末尾的 `_resolve_tool_config_refs()` 间接导入，
        # 在模块级引入就闭成环 → `config.loader` 半初始化 → ImportError。
        # 该 ImportError 被 `schema.py` 末尾的 `except ImportError` 吞掉，
        # 于是 `Config` 永远 "not fully defined"，症状出现在
        # `tests/config/test_model_presets.py` 的时区子进程用例里。
        from nanobot.agent.plugins import default_user_plugins_dir
        ...
        return cls(
            ...
            skill_overrides=defaults.skill_overrides,
            # 用户级技能目录是**产品路径**（~/.nanowork/skills），不是工作区内的路径。
            # 只在 from_config 这条生产装配路径上注入，单测构造 Agent 时不带，
            # 这样 `list_skills` 的结果不会随开发者本机装了什么技能而变。
            user_skills_dir=default_user_skills_dir(),
            # 用户级插件目录同理（~/.nanowork/plugins），只在生产装配路径注入：
            # 单测构造 Agent 时不带，插件扫描结果就不会随开发者本机装了什么而变。
            user_plugins_dir=default_user_plugins_dir(),
    ...
    def _register_default_tools(
        ...
        *,
        ...
        ctx = ToolContext(
            ...
            # [LOCAL PATCH] FR-3.4：与 ContextBuilder/SkillsLoader 同源。
